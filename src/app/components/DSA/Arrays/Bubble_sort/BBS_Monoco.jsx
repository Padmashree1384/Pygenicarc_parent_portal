...
import { BBScodeSnippets } from './BBScodeSnippets';
import {
  Box,
  Typography,
  Tabs,
...
const handleCodeAction = (action, code, language, showSnackbar) => {
  switch (action) {
    case 'download':
      ...
      link.href = url;
      link.download = `bubble_sort_code.${getFileExtension(language)}`;
...
const BBS_Monoco = ({ showSnackbar }) => {
  const [language, setLanguage] = useState('python');
  const [code, setCode] = useState(BBScodeSnippets['python']);

  useEffect(() => {
    const selectedCode = BBScodeSnippets[language];
      ...
      >
        <Box sx={{ p: { xs: 2, md: 4 }, textAlign: 'center' }}>
          ...
          <Typography variant="body1" sx={{ mb: 3, color: '#94a3b8', maxWidth: '700px', mx: 'auto' }}>
            Select a language to see its Bubble Sort implementation. Feel free to download or copy the code.
          </Typography>
        </Box>
