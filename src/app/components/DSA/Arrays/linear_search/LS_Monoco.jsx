...
import { LScodeSnippets } from './LScodeSnippets';
import {
  Box,
  Typography,
  Tabs,
...
const handleCodeAction = (action, code, language, showSnackbar) => {
  switch (action) {
    case 'download':
      ...
      link.href = url;
      link.download = `linear_search_code.${getFileExtension(language)}`;
...
const LS_Monoco = ({ showSnackbar }) => {
  const [language, setLanguage] = useState('python');
  const [code, setCode] = useState(LScodeSnippets['python']);

  useEffect(() => {
    const selectedCode = LScodeSnippets[language];
      ...
      >
        <Box sx={{ p: { xs: 2, md: 4 }, textAlign: 'center' }}>
          ...
          <Typography variant="body1" sx={{ mb: 3, color: '#94a3b8', maxWidth: '700px', mx: 'auto' }}>
            Select a language to see its Linear Search implementation. Feel free to download or copy the code.
          </Typography>
        </Box>
