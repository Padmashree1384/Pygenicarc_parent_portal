...
const aestheticTheme = createTheme({
  palette: {
    ...
    secondary: { main: '#5dade2' }, // Current Minimum
    background: {
...
const SLSLab = ({ showSnackbar }) => {
    ...
    const sketch = (p) => {
      ...
      const drawArray = (p) => {
        ...
        for (let k = 0; k < n; k++) {
          ...
          else if (k === minIndex) barColor = p.color(aestheticTheme.palette.secondary.main);
  ...
  const performStep = useCallback(() => {
    ...
    if (s.j < s.arr.length) {
      ...
      if (s.minIndex !== s.i) {
        messageBody = `Pass ${s.i + 1} complete. Swapping minimum ${s.arr[s.minIndex]} with ${s.arr[s.i]}.`;
      ...
      <Paper sx={styles.container}>
              ...
              <Grid item xs={12}>
                ...
                <Stack direction="row" spacing={1} sx={{ overflowX: 'auto', pb: 1 }}>
                  {manualArrayInputs.map((val, i) => (
                    <TextField
                      key={i} size="small" type="number" value={val}
                      onChange={e => {
                      ...
                      sx={{ minWidth: 60 }} inputProps={{ min: 1, max: 100 }}
                    />
                  ))}
                </Stack>
        ...
        <Box sx={{ mb: 2, display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%' }}>
          <Stack direction="row" spacing={2} sx={{ p: 1.5, borderRadius: 2, background: 'rgba(255,255,255,0.7)', flexWrap: 'wrap', justifyContent: 'center' }}>
            ...
            <LegendItem color={aestheticTheme.palette.secondary.main} text="Current Minimum" />
              ...
              <Stack direction="row" justifyContent="space-between" alignItems="center">
                ...
                <Stack direction="row" spacing={0.5}>
                  ...
                  <Tooltip title={isPlaying ? "Pause" : "Run"}>
                    <IconButton size="small" onClick={handleRun} disabled={isAnimating} sx={{ background: isPlaying ? aestheticTheme.palette.warning.main : aestheticTheme.palette.success.main }}>
