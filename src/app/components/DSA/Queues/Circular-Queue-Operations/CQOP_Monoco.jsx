...
import { CQOPcodeSnippets } from './CQOPcodeSnippets';
import {
  Box,
  Typography,
  Tabs,
...
const handleCodeAction = (action, code, language, showSnackbar) => {
  switch (action) {
    case 'download':
      ...
      link.href = url;
      link.download = `circular_queue_code.${getFileExtension(language)}`;
...
const CQOP_Monoco = ({ showSnackbar }) => {
  const [language, setLanguage] = useState('python');
  const [code, setCode] = useState(CQOPcodeSnippets['python']);

  useEffect(() => {
    const selectedCode = CQOPcodeSnippets[language];
      ...
      >
        <Box sx={{ p: { xs: 2, md: 4 }, textAlign: 'center' }}>
          ...
          <Typography variant="body1" sx={{ mb: 3, color: '#94a3b8', maxWidth: '700px', mx: 'auto' }}>
            Select a language to see its Circular Queue implementation. Feel free to download or copy the code.
          </Typography>
        </Box>
