...
import { INPOcodeSnippets } from './INPOcodeSnippets'; // Using INPO data source
import {
  Box,
  Typography,
  Tabs,
...
// Helper function modified to accept showSnackbar from props
const handleCodeAction = (action, code, language, showSnackbar) => {
  switch (action) {
    case 'download':
      ...
      link.href = url;
      link.download = `infix_to_postfix_code.${getFileExtension(language)}`; // Specific Filename
...
// The component now accepts the 'showSnackbar' function as a prop.
const INPO_Monoco = ({ showSnackbar }) => {
  const [language, setLanguage] = useState('python');
  const [code, setCode] = useState(INPOcodeSnippets['python']); // Using INPO data source

  // Internal snackbar state has been removed.

  useEffect(() => {
    const selectedCode = INPOcodeSnippets[language]; // Using INPO data source
      ...
      >
        <Box sx={{ p: { xs: 2, md: 4 }, textAlign: 'center' }}>
          ...
          <Typography variant="body1" sx={{ mb: 3, color: '#94a3b8', maxWidth: '700px', mx: 'auto' }}>
            Select a language to see its Infix to Postfix implementation. Feel free to download or copy the code.
          </Typography>
        </Box>
        ...
        {/* The Snackbar JSX has been removed from this component. */}
      </Paper>
    </Fade>
  );
};

export default INPO_Monoco;