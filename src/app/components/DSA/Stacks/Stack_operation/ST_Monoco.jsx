...
import { STOPcodeSnippets } from './STOPcodeSnippets';
import {
  Box,
  Typography,
  Tabs,
...
const handleCodeAction = (action, code, language, showSnackbar) => {
  switch (action) {
    case 'download':
      ...
      link.href = url;
      link.download = `stack_op_code.${getFileExtension(language)}`;
...
const ST_Monoco = ({ showSnackbar }) => {
  const [language, setLanguage] = useState('python');
  const [code, setCode] = useState(STOPcodeSnippets['python']);

  // Internal snackbar state has been removed as it is now managed by the parent component.

  useEffect(() => {
    const selectedCode = STOPcodeSnippets[language];
      ...
      >
        <Box sx={{ p: { xs: 2, md: 4 }, textAlign: 'center' }}>
          ...
          <Typography variant="body1" sx={{ mb: 3, color: '#94a3b8', maxWidth: '700px', mx: 'auto' }}>
            Select a language to see its Stack implementation. Feel free to download or copy the code.
          </Typography>
        </Box>
