...
import { VPcodeSnippets } from './VPcodeSnippets'; // Using VP data source
import {
  Box,
  Typography,
  Tabs,
...
const handleCodeAction = (action, code, language, showSnackbar) => {
  switch (action) {
    case 'download':
      ...
      link.href = url;
      link.download = `algorithm_code.${getFileExtension(language)}`; // Generic Filename
...
const VP_Monoco = ({ showSnackbar }) => {
  const [language, setLanguage] = useState('python');
  const [code, setCode] = useState(VPcodeSnippets['python']); // Using VP data source

  // Internal snackbar state has been removed.

  useEffect(() => {
    const selectedCode = VPcodeSnippets[language]; // Using VP data source
      ...
      >
        <Box sx={{ p: { xs: 2, md: 4 }, textAlign: 'center' }}>
          ...
          <Typography variant="body1" sx={{ mb: 3, color: '#94a3b8', maxWidth: '700px', mx: 'auto' }}>
            Select a language to see its implementation. Feel free to download or copy the code.
          </Typography>
        </Box>
