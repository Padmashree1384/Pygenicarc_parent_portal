...
const DLS_EX1 = () => {
  ...
  const setupTree = (p) => {
    ...
    A.children.push(B, C); B.children.push(D, E); C.children.push(F, G);
    D.children.push(H, I); E.children.push(J, K); F.children.push(L); G.children.push(M);

    nodes.current = [A, B, C, D, E, F, G, H, I, J, K, L, M];
    root.current = A;
    ...
    const w = p.width;
    const h = p.height;
    layoutTree(root.current, w / 2, h * 0.15, w / 4, h / 5);
