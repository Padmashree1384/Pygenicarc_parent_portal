...
export default function ParcVerticalNavExpansionPanel({ item, children, mode }) {
  ...
  useEffect(() => {
    // This effect handles the auto-expansion logic
    // It uses the `item.children` prop directly, which is the correct React way.
    if (item.children) {
      const isAnyChildActive = item.children.some((child) => child.path === pathname);
      if (isAnyChildActive) {
        setCollapsed(false);
      }
    }
  ...
  useEffect(() => {
    // This effect handles the animation height calculation
